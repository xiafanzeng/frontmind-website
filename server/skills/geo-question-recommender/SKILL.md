...
8. Ground every question in one or more knowledge-base paths. Do not invent a competitor, certification, price, performance claim, ranking, product, function, scenario, or customer outcome.
9. Include the current company or brand in all five `competitor_comparison` questions through `enterpriseAnchor`. Every item must also declare a different `competitorAnchor` containing an explicit competitor company or brand name found in the ZIP. Reproduce both declared anchors exactly in every visible question. Do not use a generic product or solution comparison in this category.
...
All five questions must explicitly name the current enterprise or brand through `enterpriseAnchor` and a different competitor company or brand through `competitorAnchor`. Prefer five different competitor brands when the ZIP supports them. If the ZIP contains only one named competitor, five questions may compare the same pair only when they use five genuinely different customer decisions. Generic labels such as “同类平台”“传统方案”“原生接口” or “自建集群” are not allowed as comparison targets.
...
- For all five `competitor_comparison` items, include `enterpriseAnchor` with the current sourced enterprise or brand name and a different `competitorAnchor` with a sourced competitor company or brand name. Reproduce both anchors exactly in the visible question. Never use the current enterprise, a product type, or a generic alternative as `competitorAnchor`.
...
- If the ZIP contains no named competitor, do not fabricate one. The knowledge-base input must first be regenerated with D08 competitor evidence because five valid named-brand comparison questions cannot otherwise be produced.
