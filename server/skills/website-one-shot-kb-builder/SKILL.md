...
| Customer-visible formal knowledge narrative         | evidence-adaptive; target 18,000–28,000; hard maximum 40,000 characters |
...
6. Discover image candidates before downloading files. Inspect `img`, `srcset`, lazy-load attributes, `picture/source`, CSS backgrounds, Open Graph metadata, official galleries and official-document images; list all seven discovery-method categories in the manifest even when a category yields no candidate. Keep a per-candidate ledger with the direct URL, source page, discovery method and `eligible`, `rejected`, or `uninspected` status. Select first-party images in this order: Logo/brand identity, core products or services, application scenarios, technology or manufacturing capability, certifications, then team. Give every core product family with a suitable official asset at least one image. Deduplicate by SHA-256. Treat 36–48 as a quality target, not a minimum: package every eligible asset up to 48; fewer images are a valid `source_limited` result only after all discovered candidates are inspected, and `budget_limited` requires honest uninspected candidate records. Save no more than three preview images per content branch and about 21 preview images across the page; retain remaining package capacity only for high-value first-party assets in the paginated media library. For third-party images, record the page URL, direct asset URL, source and ownership status—do not download the file or use it to fill the target.
...
- **Formal customer copy first.** Every display branch has exactly one `overview` document written as finished enterprise knowledge in addition to 40–56 true `leaf` documents. Do not present crawl mechanics, “第一方原始快照”, “第一方页面摘录”, raw snapshots, page excerpts or source boilerplate as the formal narrative. Move acquisition details and verbatim evidence excerpts to non-customer-visible `evidence` documents and link them with `evidenceDocumentIds`. Deduplicate evidence documents by normalized content hash and link only evidence with the same canonical `branchId`; related leaves in that branch may reuse the same evidence ID.
...
- **Evidence-adaptive narrative quality.** Target 18,000–28,000 effective customer-visible characters and never exceed 40,000; do not impose a total minimum. Classify each display branch as `complete`, `limited_evidence`, or `needs_verification`. For an overview with linked deduplicated evidence, require `min(target floor, max(120, ceil(actual evidence characters × 25%)))`, where the target floor is 1,500 for ordinary branches and 3,000 for products/services. For a leaf, require `min(200, max(60, ceil(actual linked evidence characters × 20%)))`. With no evidence, write at least 40 effective characters that name the checked scope and exact gap and use `needs_verification` or reasoned `not_applicable`. Never declare zero evidence while linked evidence exists; both validators recalculate evidence characters from the actual linked evidence documents. Do not repeat the same template paragraph across leaves.
...
Enter this phase automatically after every stable leaf is written or reasoned `not_applicable`. Generate the bounded ZIP with hierarchical folders, seven formal Markdown overviews plus 40–56 true leaves, linked evidence documents, selected first-party images, URL-only third-party references, README, final knowledge-tree status, both acquisition reports, the unchanged raw `00_completeness.json`, the schema-v2 `00_package_manifest.json`, image candidate ledger and unresolved verification gaps. The final ZIP must contain at most 150 files, at most 48 saved images and at most 40,000 effective customer-visible narrative characters. It must also stay within the 220 MB uncompressed, 8 MB per-document and 200:1 per-entry safety limits, use only the allowlisted types in `references/output-format.md`, contain no symbolic link, and have no case- or Unicode-normalization-equivalent duplicate path. Do not include per-page raw HTML or duplicate cleaned-page files.
