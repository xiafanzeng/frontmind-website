...
For brand-named reputation, complaint, defect, risk, or reliability questions, the prompt itself forces a brand mention. Preserve this exclusion in the forecast assumptions or limitations. The fixed v2 transfer object still carries all thirteen `projectable` action-and-evidence mappings so it remains structurally deterministic; these entries are not permission to publish excluded visibility, multi-platform brand coverage, first-mention, rank, Top3, or Top5 improvement. The service must suppress those ranges after validating the transfer object. `exclusiveSemanticSpace` remains distinct from ranking and may survive service-side suppression only when the baseline measured or derived verified differentiator clarity, the knowledge base supplies the evidence set, and the execution scenario contains mapped positioning, Q&A, or distribution actions. Coherence, factual accuracy, answer completeness, authority, and knowledge gaps may still be assessed where evidence supports them.
