...
Immediately before submission, compare the completed object recursively with the attached `optimization-forecast-output-template.json`: the key paths and fixed thirteen-indicator structure must be identical, `schemaVersion` must be the number `2`, every template placeholder must be filled as required by the schema, and no v1, prose-only, missing-key, or extra-key variant may be returned. Then serialize and parse it once and run the schema check on that parsed value.
