...
10. Validate against `references/output-schema.json`, save the completed object as `optimization-forecast-result.json`, and attach that single file as a typed assistant `output_file` with `application/json`. If the active model channel cannot create an `output_file`, return the same complete object inline as the final assistant `output_text`.
...
Prefer one typed `output_file` named `optimization-forecast-result.json`; do not merely mention a filename, path, or link in text. The file must contain only the completed JSON object, use UTF-8, and remain below 4 MiB. If file output is unavailable, the fallback `output_text` must contain only that JSON object: its first non-whitespace character is `{` and its last non-whitespace character is `}`. Do not add acknowledgements or validation claims before or after the result. If output length is constrained, shorten optional customer-facing wording while preserving every required field. Treat local validation as preflight only—the server is the final validation authority.
