...
const COMPETITOR_COMPARISON_PATTERN =
  /(?:对比|相比|比较|区别|差异|不同|相较|取舍|还是|同类|传统方案|传统工具|自建|替代|与.+哪个|和.+哪个|(?:如何|怎么)评估.+(?:与|和|跟).+|评估.+(?:与|和|跟).+(?:关注|考察|考虑|权衡)|vs)/i;
...
export function isIndustryRankingQuestion(question: string) {
  ...
  // A comparison between named alternatives remains a selectable competitor
  // comparison even when it asks which one is better.
  const namedComparison =
    /(?:和|与|跟|对比|相比|vs).{0,30}(?:哪个好|哪家好|更好|更适合|优劣|区别)/.test(
      normalized,
    );
