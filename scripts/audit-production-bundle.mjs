...
if (
  resolveProductionBuildSource({
    repositoryRoot: projectRoot,
    env: process.env,
    expectedBuildSha: buildSourceSha,
  }) !== buildSourceSha
) {
  throw new Error("BUILD_SOURCE_IDENTITY_MISMATCH");
}
...
console.log(
  `Production image content audit passed: source=${buildSourceSha} files=${artifactManifest.files.length} root=${artifactManifest.rootSha256}`,
);
