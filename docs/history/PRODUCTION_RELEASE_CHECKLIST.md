# FrontMind Agent + Website 生产发布清单（历史文档，禁止用于常规发布）

> **已废弃：** 本文记录的是源码挂载、`dist` 双提交与人工验收时代的首次上线过程，
> 不再是生产发布入口。常规发布及数据库变更一律遵循 Dashboard 仓库的
> [权威发布手册](https://github.com/xiafanzeng/frontmind-dashboard/blob/main/docs/operations/RELEASE.md)。
> Website 的镜像、签名、探针和受限部署接口见
> [`docs/website-release-contract.md`](../website-release-contract.md)。本文仅可用于历史审计，
> 不得复制其中的 S/F、artifact root、服务器安装依赖或源码挂载命令。

以下内容曾是两仓联动的首次上线顺序，现仅保留原文供历史审计：
