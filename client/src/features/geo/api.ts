...
function rememberPendingGeoCustomQuestionValidation(
  ...
  try {
    ...
    // Server-side reservation remains authoritative when storage is unavailable.
  }
}
...
async function acknowledgeNonRetryableGeoCustomQuestionTerminal(
  ...
  try {
    ...
    // Keep the exact UUID so a later refresh can retry the idempotent ACK.
  }
  return true;
}
...
> {
  ...
  try {
    if (pending) {
      const retiredOperation = pending;
      try {
        payload = await requestJson(
          `${basePath}/${encodeURIComponent(retiredOperation.clientRequestId)}`,
          { signal: options.signal },
        );
      } catch (error) {
        if (
        ...
        try {
          ...
          if (
            !(activeError instanceof GeoApiError) ||
            activeError.code !== "CUSTOM_QUESTION_VALIDATION_NOT_FOUND"
          ) {
            throw activeError;
          }
          clearPendingGeoCustomQuestionValidation(
            project.id,
            retiredOperation.clientRequestId,
          );
          throw new GeoApiError(
            "之前的问题验证记录已过期，请重新提交。",
            410,
        ...
        const recoveredValidation = asRecord(asRecord(payload).validation);
        const recoveredClientRequestId = textValue(
          recoveredValidation.clientRequestId,
        );
        const recoveredQuestion = textValue(recoveredValidation.question);
        if (
          !isGeoCustomQuestionClientRequestId(recoveredClientRequestId) ||
          !recoveredQuestion
        ) {
          throw new GeoApiError(
            "待恢复的问题验证状态无效，请刷新后重试。",
