/* Blog routes intentionally live in their own async boundary so the legacy
 * community catalog is not downloaded with every article or index visit. */
...
const blogContentPaths = new Set([
  "/blogs",
  ...communityPostsCn.map((post) => postPath(post.slug)),
]);
...
export default function GeoCommunityPage() {
  ...
  const canonicalPath = canonicalBlogCommunityPath(communityPath);
  ...
  const title = isBlogIndex
    ...
    : post?.titleCn || t("未收录文章", "Missing Article");
  const metaTitle = post
    ? `${post.titleCn} - FrontMind GEO 社区`
    : `FrontMind - ${title}`;
...
function CommunityBlogIndex({ query }: { query: string }) {
  ...
  const filteredPosts = communityPostsCn.filter(
    (post) => category === "all" || post.categoryCn === category,
  );
        ...
        <div className="container">
          ...
          <div className="max-w-3xl">
            ...
            >
              {t(
                "GEO 研究与学习社区文章库",
                "GEO Research and Learning Library",
              )}
            </h1>
            <p
            ...
            >
              {t(
                "这里汇集 143 篇中文整理后的 GEO、AI 搜索、LLM 评测、技术 SEO 与内容策略文章。文章按分页展示，方便逐步阅读和检索。",
                "This library brings together 143 localized articles on GEO, AI search, LLM evaluation, technical SEO, and content strategy with paginated browsing.",
              )}
            </p>
          </div>
        </div>
      ...
      <section className="py-12 md:py-16">
        <div className="container">
          ...
          <div className="mb-7 flex flex-col justify-between gap-3 border-b border-[#e5e7eb] pb-5 md:flex-row md:items-end">
            <div>
              <p
                className="text-xs font-bold uppercase tracking-[0.2em] text-[#C5A24D]"
                style={{ fontFamily: "'DM Sans', sans-serif" }}
              >
                {t("全部文章", "All Articles")}
              </p>
              <h2
                className="mt-2 text-2xl font-bold text-[#1A1A2E] md:text-3xl"
                style={{ fontFamily: "'DM Serif Display', serif" }}
              >
            ...
            >
              {t(
                `共 ${filteredPosts.length} 篇，每页 ${BLOG_PAGE_SIZE} 篇`,
                `${filteredPosts.length} posts, ${BLOG_PAGE_SIZE} per page`,
              )}
            </p>
          </div>
...
function CommunityPostCard({ post }: { post: CommunityPostCn }) {
  return (
    <Link
      href={communityPathToFrontMind(postPath(post.slug))}
      className="fm-card group grid overflow-hidden no-underline md:grid-cols-[1.28fr_0.82fr]"
    >
      ...
      <div className="flex flex-col p-5 md:p-6">
        <div className="mb-4 flex flex-wrap items-center gap-2">
          <span
            className="rounded-full bg-[#3D1560]/7 px-3 py-1 text-xs font-bold text-[#3D1560]"
            style={{ fontFamily: "'DM Sans', sans-serif" }}
          >
            {post.categoryCn}
          </span>
        ...
        <h2
          className="mb-3 text-xl font-bold leading-tight text-[#1A1A2E] group-hover:text-[#3D1560]"
          style={{ fontFamily: "'DM Serif Display', serif" }}
        >
          {post.titleCn}
        </h2>
        ...
        <div
          className="mt-auto flex flex-wrap items-center gap-4 pt-5 text-xs text-[#9CA3AF]"
          style={{ fontFamily: "'DM Sans', sans-serif" }}
        >
          <span className="inline-flex items-center gap-1">
            <Calendar size={12} /> {post.dateCn}
          </span>
        </div>
      </div>
    </Link>
...
function Pagination({
  ...
  return (
    ...
    >
      {page > 1 && (
        <a
          href={blogIndexHref(category, page - 1)}
          className="rounded-md border border-[#3D1560]/15 bg-white px-4 py-2 text-sm font-semibold text-[#3D1560] no-underline hover:bg-[#3D1560]/5"
        >
          上一页
        </a>
      )}
        ...
        <div className="container">
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 lg:items-center">
            <div className="lg:col-span-7">
              ...
              <p
                className="mb-6 text-base leading-relaxed text-[#6B7280] md:text-lg"
                style={{ fontFamily: "'DM Sans', sans-serif" }}
              >
                {post.metaCn}
              </p>
              <div
                className="flex flex-wrap items-center gap-4 text-sm text-[#9CA3AF]"
                style={{ fontFamily: "'DM Sans', sans-serif" }}
              >
                <span>{post.categoryCn}</span>
                <span className="inline-flex items-center gap-1">
                  <Calendar size={14} /> {post.dateCn}
                </span>
              </div>
            </div>
            ...
            <article className="fm-card p-6 md:p-10">
              {post.tldrCn.length > 0 && (
                <div className="mb-10 rounded-lg border border-[#3D1560]/10 bg-[#FBFAFC] p-5">
                  <h2
                    className="mb-4 text-xl font-bold text-[#1A1A2E]"
                    style={{ fontFamily: "'DM Serif Display', serif" }}
                  >
...
function CommunityMissingArticle() {
  const { t } = useLang();
...
function postForPath(path: string) {
  const slug = path.match(
    /^\/blogs\/generative-engine-optimization\/([^/]+)\/?$/,
  )?.[1];
...
function postImage(post: CommunityPostCn) {
  const file = (post.imageCn.split("/").pop() || "").replace(
    /\.(png|jpe?g)$/i,
    ".webp",
  );
...
function researchPageHrefForPost(post: CommunityPostCn) {
  ...
  return page <= 1
    ? "/research#geo-community"
    : `/research?page=${page}#geo-community`;
}
...
function blogCategories() {
  const counts = communityPostsCn.reduce<Record<string, number>>(
    (acc, post) => {
